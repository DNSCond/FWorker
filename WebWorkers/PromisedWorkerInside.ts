...
export class PromisedWorkerInside {
    ...
    protected eventListener_: number | null;

    constructor(settings?: { timeout: number }) {
    ...
    eventListener(event: MessageEvent) {
        ...
        if (uuid) {
            ...
            try {
                result_Sync = callWith(eventListener, promisedWorker,
                    [event.data.message, resolve, reject]);
    ...
    setMessageEventListener<SEND, RETURN>(eventListener: (data: SEND) => Promise<RETURN> | RETURN): boolean {
...
export function callWith(callable: Function, thisContext: any, ...args: any[]) {
