...
export class PromisedWorker {
    ...
    awaitMessage(message, timeout = 100_000) {
        const { promise, resolve, reject } = Promise.withResolvers(), uuid = self.crypto.randomUUID();
        if (this.jobs.has(uuid))
            throw new RangeError("UUID collision detected");
...
export class TerminationError extends CustomError {
    constructor(message) {
        super(message, null);
    }
    get [Symbol.toStringTag]() {
